...
async function safeFetch(url, options = {}, button = null) {
    ...
    const key = DouyinRequestManager.generateKey(url, method, options.body);
    
    // 检查重复请求
    if (DouyinRequestManager.isPending(key)) {
        console.warn('请求正在进行中，已拦截重复请求:', url);
        showToast('操作进行中，请勿重复点击', 'warning');
        throw new Error('DUPLICATE_REQUEST');
    }
    
    DouyinRequestManager.startRequest(key);
    ...
    try {
        ...
        DouyinRequestManager.endRequest(key);
